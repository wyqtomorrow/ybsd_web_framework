...
var temp = "{{if items}}"
    ...
    + '					<p class="d_blue">'
    + '						<span class="iconfont icon-zhifumaijiashu2 f20 valign"></span>买家：{{$value.memberResForm.realName}}'
    + '					</p>'
    + '					<p class="black3">'
...
mui("body").on("tap", "#wancheng", function () {
    self.location.href = "/mob/order/orderSellList.do?sign=" + "1";
})

mui("body").on("tap", "#jinxing", function () {
    self.location.href = "/mob/order/orderSellList.do";
})
//取消该订单
...
mui("body").on("tap", "#orderDetail", function () {
    var idOrder = $(this).parents("li").attr("data-id");
    self.location.href = contextPath + "/mob/order/orderDetail?orderId=" + idOrder + "&sign=" + "1";
})
