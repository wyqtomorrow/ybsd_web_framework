...
$("#publish_info").on("tap", function () {
    ...
    attrNl.val = $("#age").val();
    attrNl.attrDefId="GAV_MONTH_AGE";
...
(function (mui, doc) {
    ...
    mui.ready(function () {
        ...
        /*var userPicker8 = new mui.PopPicker({layer:3});
        userPicker8.setData(cityData3);
        var cd = doc.getElementById('cd');
        var chandi=doc.getElementById("place");
        var tval8=doc.getElementById("tval8");
        cd.addEventListener('tap', function(event) {
            userPicker8.show(function(items) {
                chandi.value = items[0].text+items[1].text+items[2].text;
                tval8.value = items[2].value;
            });
        }, false);*/
